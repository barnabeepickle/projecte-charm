...
public class ItemProjectE extends Item {
    ...
    protected static final ResourceLocation ACTIVE_NAME = new ResourceLocation(Tags.MODID, "active");
    protected static final IItemPropertyGetter ACTIVE_GETTER = (stack, world, entity) -> stack.hasTagCompound() && Objects.requireNonNull(stack.getTagCompound()).getBoolean(TAG_ACTIVE) ? 1F : 0F;

    public ItemProjectE() {
        this.setCreativeTab(ObjHandler.cTab);
    }

    public ItemProjectE(CreativeTabs tab) {
    ...
    public Item setTranslationKey(@Nonnull String message) {
        return super.setTranslationKey(message);
    }
    ...
    public boolean shouldCauseReequipAnimation(ItemStack oldStack, ItemStack newStack, boolean slotChanged) {
        ...
        boolean diffActive = oldStack.hasTagCompound() && newStack.hasTagCompound()
                && Objects.requireNonNull(oldStack.getTagCompound()).hasKey(TAG_ACTIVE) && Objects.requireNonNull(newStack.getTagCompound()).hasKey(TAG_ACTIVE)
        ...
        boolean diffMode = oldStack.hasTagCompound() && newStack.hasTagCompound()
                && oldStack.getTagCompound().hasKey(TAG_MODE) && Objects.requireNonNull(newStack.getTagCompound()).hasKey(TAG_MODE)
